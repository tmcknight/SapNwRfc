...
namespace SapNwRfc.Internal.Fields
{
    ...
    internal sealed class TimeOnlyField : Field<TimeOnly?>
    {
        ...
        public TimeOnlyField(string name, TimeOnly? value)
            : base(name, value)
        {
        }
        ...
        public static TimeOnlyField Extract(RfcInterop interop, IntPtr dataHandle, string name)
        {
            ...
            if (timeString == EmptyRfcTimeString || timeString == ZeroRfcTimeString)
                return new TimeOnlyField(name, null);

            if (!TimeOnly.TryParseExact(timeString, RfcTimeFormat, out TimeOnly time))
                return new TimeOnlyField(name, null);

            return new TimeOnlyField(name, time);
        }
